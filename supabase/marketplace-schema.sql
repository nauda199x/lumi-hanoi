...
create table if not exists public.listings (
  ...
  unit_type text not null check (unit_type in ('1PN','2PN','3PN','4PN','Duplex','Penthouse')),
  ...
  available_from date,
  legal_status text,
  poster_type text not null check (poster_type in ('owner','agent')),
...
create index if not exists listings_public_feed_idx on public.listings(listing_type,status,is_featured desc,sort_priority desc,approved_at desc);
create index if not exists listings_location_idx on public.listings(phase,tower,unit_type,price_vnd);
...
create or replace function public.can_attach_pending(target_listing_id uuid)
returns boolean
language sql
stable
security definer
...
revoke all on function public.can_attach_pending(uuid) from public;
grant execute on function public.can_attach_pending(uuid) to anon, authenticated;

create or replace function public.can_upload_pending_image(object_name text)
returns boolean
language plpgsql
stable
security definer
...
begin
  ...
  if not public.can_attach_pending(target_listing_id) then
    return false;
  end if;
  return (
    select count(*) < 12
...
revoke all on function public.can_upload_pending_image(text) from public;
grant execute on function public.can_upload_pending_image(text) to anon, authenticated;
...
create policy listings_admin_manage on public.listings for all to authenticated using (public.is_admin()) with check (public.is_admin());
...
create policy listing_images_anon_insert on public.listing_images for insert to anon with check (public.can_attach_pending(listing_id));

drop policy if exists listing_images_admin_manage on public.listing_images;
create policy listing_images_admin_manage on public.listing_images for all to authenticated using (public.is_admin()) with check (public.is_admin());
...
create policy listing_reports_admin_manage on public.listing_reports for all to authenticated using (public.is_admin()) with check (public.is_admin());

revoke all on public.admin_users,public.listings,public.listing_images,public.listing_reports from anon;
grant select (
  id,listing_code,slug,listing_type,status,title,description,phase,tower,unit_type,bedroom_count,
  area_sqm,floor_label,price_vnd,furnishing,direction,view_text,available_from,legal_status,
  poster_type,contact_phone,contact_zalo,is_featured,sort_priority,approved_at,expires_at,created_at,updated_at
) on public.listings to anon;
grant insert (
  id,listing_code,slug,listing_type,title,description,phase,tower,unit_type,bedroom_count,
  area_sqm,floor_label,unit_code,price_vnd,furnishing,direction,view_text,available_from,legal_status,
  poster_type,poster_name,contact_phone,contact_zalo,contact_email,contact_public
) on public.listings to anon;
...
grant all on public.admin_users,public.listings,public.listing_images,public.listing_reports to authenticated;
...
create policy listing_images_storage_public_read on storage.objects for select to anon,authenticated using (bucket_id = 'listing-images');
...
with check (
  ...
  public.can_upload_pending_image(name) and
