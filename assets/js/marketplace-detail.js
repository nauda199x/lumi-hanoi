(()=>{
  ...
  const load=async()=>{if(!identifier){showMissing("Không tìm thấy mã tin","Đường dẫn này chưa có mã tin hợp lệ.");return;}if(!api.configured()){showMissing("Dữ liệu giao dịch đang được cập nhật","Vui lòng quay lại sau khi hệ thống hoàn tất cập nhật.");return;}try{const listing=await api.getPublicListing(identifier);if(!listing)showMissing("Tin không còn hiển thị","Tin có thể đang chờ duyệt, đã hết hạn hoặc đã giao dịch.");else render(listing);}catch{showMissing("Chưa thể tải tin đăng","Vui lòng kiểm tra kết nối và tải lại trang.");}};
  load();
})();
